...
def get_chip_contour(img): # ищет прямоугольник на выделенном с фото с помощью маски чипе
    ...
    for cnt in contours:
        ...
        if area > 1000:
            ...
            #printapprox_points)
            cv2.imwrite("chip_boundaries.jpg", img)
            ####cv2.waitKey(0)
            ...
            cv2.imwrite("chip_norm_pos.jpg", cropped_chip_jpg)
            #cv2.waitKey(0)
...
photo_num = "1"
path = "Resources\photo (" + photo_num + ").jpg"
img = cv2.imread(path)
img_orig = cv2.resize(img, (width_img, height_img))
img = cv2.resize(img, (width_img, height_img))
img_contour = img.copy()

# chip image size - 654x390
...
chip_img = cv2.imread("chip_norm_pos.jpg")
cv2.imwrite("contour.jpg", img_contour)

#resistor_placement=pd.read_csv('resistor_placement.csv', sep=',',header=None)
resistor_placement = genfromtxt('resistor_placement.csv', delimiter=';')
